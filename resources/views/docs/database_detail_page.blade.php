        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
            ...
            <div class="md:col-span-2">
                ...
                <div class="daftar-gambar flex flex-wrap gap-4">
                    ...
                    @foreach ($databaseImg as $index => $img)
                        <div class="relative group w-fit">
                        <img src="{{ asset($img->link) }}"
                            alt="Gambar UAT"
                            class="max-h-40 rounded shadow cursor-pointer preview-modal-img"
                            data-img-index="{{ $index }}"
                            data-img-src="{{ asset($img->link) }}">


                            @auth
                            @if(auth()->user()->role === 'admin')
                            <form action="{{ route('databases.deleteImages', $img->id) }}" method="POST" class="absolute top-1 right-1 hidden group-hover:block">
                                @csrf
                                @method('DELETE')
                ...
                {{-- FORM Tambah gambar --}}
                    <form action="{{ route('databases.storeImages') }}" method="POST" enctype="multipart/form-data">
                        @csrf

                        <input type="hidden" name="databases_id" value="{{ $databaseData->id_database }}">
